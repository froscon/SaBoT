...
class ObjectPermCheckGETMixin(object):
    def get(self, request, *args, **kwargs):
        ...
        if (
            not self.object.has_read_permission(self.request.user)
...
class ParticipantsView(ObjectPermCheckMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["participants_list"] = (
            self.connection_table_class.objects.select_related()
    ...
    def form_valid(self, form):
        ...
        try:
            try:
                ...
                user = User.objects.annotate(profiles=Count("legacy_profile")).get(
                    email__iexact=email, profiles=0
                )
            # just store the user in the connection table
            # check if this user is already connected
            res = self.connection_table_class.objects.filter(
                project=self.object, user=user
            ).count()
            if res == 0:
            ...
            if len(User.objects.filter(username__iexact=proposedname)) > 0:
                for num in range(1, 101):
                    if (
                        len(
                            User.objects.filter(
                                username__iexact=proposedname + str(num)
                            )
                        )
                        == 0
                    ):
    ...
    def send_info_email(self, user):
        ...
        subject = render_to_string(
            "registration/activation_email_autocreate_subject.txt", ctx_dict
        )
        # Email subject *must not* contain newlines
        subject = "".join(subject.splitlines())

        message = render_to_string(
            "registration/activation_email_autocreate.txt", ctx_dict
        )
    ...
    def get_redirect_url(self):
        ...
        if self.redirect is None:
            raise ImproperlyConfigured(
                "You have to specify either 'next_view' or 'redirect'"
            )
...
class PropertySetterView(SingleObjectMixin, ObjectBasedRedirectMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if callable(self.property_value):
            setattr(
                self.object, self.property_name, self.property_value(request, **kwargs)
            )
        else:
...
class PermCheckPropertySetterView(PropertySetterView):
    ...
    def post(self, request, *args, **kwargs):
        self.object = self.get_object()
        if (
            not self.permission_checker(self.object, self.request.user)
            and not self.request.user.is_staff
        ):
            raise PermissionDenied
...
class JobProcessingView(TemplateResponseMixin, View):
    ...
    def get_context_data(self, **kwargs):
        return {"params": kwargs, "errors": self.job_errors}
...
class ObjectFileDownloader(SingleObjectMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            raise ImproperlyConfigured(
                "The upload field with name {} does not exist".format(self.upload_field)
            )
