...
def sendLetter(
    ...
    files = {"document": letterFile}

    r = requests.post(API_URL, data=params, files=files, verify=True)
    if r.status_code != 200:
        raise SmskaufenException(
            "API Failure. HTTP request returned with code {}. Message: {}".format(
                r.status_code, r.text
            )
        )
    if int(r.text) < 200:
        error_code = int(r.text)
        error_message = SMSKAUFEN_RETURN_VALS.get(
            error_code, "unknown error code " + r.text
        )
