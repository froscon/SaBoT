...
from exhibitor.forms import (
    ExhibitorGeneralForm,
    ExhibitorDescriptionForm,
    ExhibitorBoothForm,
)
        ...
        login_required(
            PermCheckSimpleDeleteView.as_view(
                ...
                redirect=lambda obj, kwargs: reverse(
                    "exhibitor_participants", kwargs={"pk": obj.project_id}
                ),
            )
        ),
        ...
        user_is_staff(
            EmailOutputView.as_view(
                queryset=lambda req, kwargs: User.objects.filter(
                    Q(
                        exhibitorparticipants__isAdmin=True,
