...
class OverviewView(TemplateView):
    def get_context_data(self, **kwargs):
        ...
        context["user_exhibitors"] = (
            Exhibitor.objects.filter(year=getActiveYear(self.request))
            .filter(Q(owner=self.request.user) | Q(participants=self.request.user))
            .distinct()
        )
        context["user_devrooms"] = (
            Devroom.objects.filter(year=getActiveYear(self.request))
            .filter(Q(owner=self.request.user) | Q(participants=self.request.user))
            .distinct()
        )
