...
urlpatterns = [
            ...
            MultipleListView.as_view(
                template_params={
                    "object_list": lambda req, kwargs: Sponsoring.objects.select_related().filter(
                        year=getActiveYear(req), commitment=True, clearedForBilling=True
                    ),
    ...
    path(
        "create/<int:spk>",
        user_is_finance(
            views.InvoiceCreateUpdateView.as_view(next_view="invoice_overview")
        ),
        name="invoice_create",
    ),
    path(
        "downloadinvoice/<int:pk>",
        user_is_staff(
            ObjectFileDownloader.as_view(
                model=Invoice, upload_field="pdf", content_type="application/pdf"
            )
        ),
