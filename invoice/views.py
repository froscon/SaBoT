...
class InvoiceSnailMailView(JobProcessingView):
	...
	def get_sponsoring(self, sid):
		try:
			sponsoring = Sponsoring.objects.select_related("invoice", "snailmailinvoice","contact").get(pk=sid)
		except Sponsoring.DoesNotExist:
			raise Http404
		return sponsoring
	...
	def process_job(self):
		...
		if SMSKaufenSnailMailJob.objects.filter(sponsoring=self.sponsoring).count() > 0:
			self.job_errors.append("A snailmail invoice was already created. You should not have reached this view")
			return False

		feedPath = reverse("invoice_snailmailstatus", kwargs={ "invId" : self.sponsoring.invoice.invoiceNumber })
		...
		try:
			result = smskaufen.api.sendLetter(self.sponsoring.invoice.pdf.file, color=True, feedUrl=feedUrl, international=international)
