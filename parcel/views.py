...
def queryParcelOwners(request):
    ...
    if "q" in request.GET:
        ...
        res = Sponsoring.objects.filter(
            contact__companyName__icontains=query, year=activeYear
        )
        response = response + [
            {
...
class LinkedParcelCreateView(CallableSuccessUrlMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if (
            not linked.has_write_permission(self.request.user)
...
class LinkedParcelListView(ListView):
    ...
    def linked_object(self):
        ...
        if (
            not linked.has_read_permission(self.request.user)
            and not self.request.user.is_staff
        ):
            raise PermissionDenied
        return linked
