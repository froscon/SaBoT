...
class SponsorContact(models.Model):
	...
	address2 = models.CharField(blank=True,max_length=128, verbose_name=_("Address addition"))
	...
	contactPersonLanguage = models.CharField(max_length=16, choices=LANGUAGE_CHOICES, verbose_name=_("Language of contact person"), default="de-DE")
	...
	def allDataAvailable(self):
		return (self.companyName != ""
			...
			and self.street != ""
			and self.zipcode != "" and self.zipcode != "0"
			and self.city != ""
...
class SponsorPackage(models.Model):
	...
	color = models.CharField(default="#ffffff", max_length=7, verbose_name=_("Display color of sponsor package"), validators=[ RegexValidator(regex=r"#[0-9a-f]{6}", message = _("Please enter a HTML color code of format #[0-9a-f]{6}")) ] )
	...
	boothPositionEN = models.CharField(blank=True, max_length=256, verbose_name=_("Position of booth for packet description. This text gets printed in the description. Leave empty to just generate an generic item. (EN)"))
	boothPositionDE = models.CharField(blank=True, max_length=256, verbose_name=_("Position of booth for packet description (DE)"))
	hasProgramAd = models.BooleanField(default=False,verbose_name=_("Has the sponsor an advertisement in the printed program?"))
	programAdInfo = models.TextField(blank=True,verbose_name=_("Displayed information about the size, quality, color, etc on the printed advertisement"))
	programAdInfoDescEN = models.CharField(blank=True, max_length=256, verbose_name=_("Information about advertisement in our program for the packet description (EN)"))
	programAdInfoDescDE = models.CharField(blank=True, max_length=256, verbose_name=_("Information about advertisement in our program for the packet description (DE)"))
	hasPackets = models.BooleanField(default=False,verbose_name=_("Is the sponsor allowed to send packets to us?"))
	hasParticipants = models.BooleanField(default=False,verbose_name=_("Has the sponsor the option to register VIPs? (usually for booth)"))
	hasProgramAdText = models.BooleanField(default=False,verbose_name=_("Has the sponsor an advertisement text in the printed program?"))
	hasRecruitingEvent = models.BooleanField(default=False,verbose_name=_("Has the sponsor the option to participate in the recruting event?"))
	programAdTextNumWords = models.PositiveIntegerField(blank=True,null=True, verbose_name=_("How many words is the advertisement text allowed to have?"))
	numFreeTickets = models.PositiveIntegerField(default=0, verbose_name=_("How many free tickets gets this sponsor?"))
	price = models.DecimalField(max_digits=32, decimal_places=2, verbose_name=_("Package price"))
	...
	sponsorPackageDescriptionDE = [
		("hasBooth", lambda s : s.boothPositionDE if len(s.boothPositionDE) > 0 else u"Ausstellungsplatz"),
		...
		( "hasLogoOnPrintmedia", u"Logopräsenz auf allen Werbemitteln (z. B. Badges, Besucherflyer, Plakat)"),
		( "hasSocialMedia", u"Namentliche Erwähnung auf unseren SocialMedia Kanälen (Twitter, Facebook, G+)"),
		( "hasProgramAd", lambda s : u"Anzeige im Konferenzprogramm ({})".format(s.programAdInfoDescDE)),
		( "hasProgramAdText", lambda s : u"Text im Konferenzprogramm ({} Wörter)".format(s.programAdTextNumWords)),
		( lambda s: True, lambda s : [text.strip() for text in s.additionalContentTextDE.strip().split("\n") if text.strip() != ""]),
	]
	sponsorPackageDescriptionEN = [
		("hasBooth", lambda s : s.boothPositionEN if len(s.boothPositionEN) > 0 else u"Exhibition space"),
		...
		( "hasLogoOnPrintmedia", u"Your logo on all publications (e. g. flyers, posters, badges)"),
		( "hasSocialMedia", u"Official mention of sponsorship on our social media channels (Twitter, Facebook, G+)"),
		( "hasProgramAd", lambda s : u"Advertisement in the printed conference program ({})".format(s.programAdInfoDescEN)),
		( "hasProgramAdText", lambda s : u"Text in the printed conference program ({} words)".format(s.programAdTextNumWords)),
...
class Sponsoring(models.Model):
	...
	participants = models.ManyToManyField(User,blank=True,editable=False,related_name="sponsorparticipation", through="SponsoringParticipants")
	...
	twitterAccountOptOut = models.BooleanField(default=False,verbose_name=_("Sponsor does not use a Twitter account"))
	facebookPage = models.URLField(blank=True,verbose_name=_("Facebook fanpage URL"))
	facebookPageOptOut = models.BooleanField(default=False,verbose_name=_("Sponsor does not use Facebook fanpage URL"))
	facebookAccount = models.CharField(blank=True,max_length=128,verbose_name=_("Facebook account name"))
	facebookAccountOptOut = models.BooleanField(default=False,verbose_name=_("Sponsor does not use a Facebook account"))
	...
	def boothStatus(self):
		...
		if self.wantBooth == False:
			return STATUS_NOTWANTED
		if self.boothTables is not None and self.boothChairs is not None and self.boothBarTables is not None:
			return STATUS_COMPLETE
		else:
			return STATUS_INCOMPLETE
		...
		for name, link, testF, deadline, statusF in self.inputStatusList:
			if testF(self.package):
				statusData.append({
					...
					"link" : link,
					"deadline" : deadline,
					"overdue" : False if deadline is None else (deadline < datetime.date.today()),
					"dueWarn" : (deadline - settings.DUE_WARNING_TIME < datetime.date.today()) if deadline is not None else False,
					"status" : statusF(self),
				})
	...
	inputStatusList = [
		...
		("Your website link", "#general", lambda p : True, None, lambda s : s.hpLinkStatus),
		("Logo (website)", "#general", lambda p : True, None, lambda s : STATUS_COMPLETE if s.logo else STATUS_MISSING),
		("Logo (print)", "#general", lambda p: p.hasLogoOnPrintmedia, None, lambda s : STATUS_COMPLETE if s.vectorLogo else STATUS_MISSING),
		("Billing addresss", "#billing-address", lambda p: True, None, lambda s : s.billingAddressStatus),
		("Social media accounts", "#social-media", lambda p: p.hasSocialMedia, None, lambda s : s.socialMediaStatus),
		("Homepage description texts (for exhibitor directory)", "#conference-homepage", lambda p : p.hasHpText, settings.BOOTH_DATA_DEADLINE, lambda s : s.hpTextStatus ),
		("Booth", "#booth", lambda p : p.hasBooth, settings.BOOTH_DATA_DEADLINE, lambda s : s.boothStatus),
		("Recruiting session", "#recruiting", lambda p : p.hasRecruitingEvent, settings.RECRUITING_DATA_DEADLINE, lambda s : s.recruitingStatus),
		("Parcel", "#parcel", lambda p : p.hasPackets, settings.PARCEL_DATA_DEADLINE, lambda s : s.packetsStatus),
		("Advertisement in printed program", "#printed-program", lambda p : p.hasProgramAd, settings.PRINTED_PROGRAM_DATA_DEADLINE, lambda s : STATUS_COMPLETE if s.programAd else STATUS_MISSING),
