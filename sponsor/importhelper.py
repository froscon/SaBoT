...
anonymous_template = SponsorMailTemplate.objects.get(templateName="sponsor/mails/DamenUndHerren.html")
personal_template = SponsorMailTemplate.objects.get(templateName="sponsor/mails/Sie.html")
...
def contactFromCSVLine(line):
	...
	if c.contactPersonFirstname == res.group(5):
		...
		c.comment = c.comment + "\nContact detail was: " + res.group(4) + " with form of address " + res.group(5)
...
def checkDoubleContact(c):
	...
	if c.contactPersonEmail != "":
		q = SponsorContact.objects.filter(contactPersonEmail__iexact=c.contactPersonEmail)
		if len(q) > 0:
			res.extend(list(q))
