...
urlpatterns = [
    path(
        "<int:lpk>/list",
        login_required(
            LinkedParcelListView.as_view(
                linked_model=Sponsoring, template_name="sponsor/tracking.html"
            )
        ),
    ...
    path(
        "<int:lpk>/new",
        login_required(
            LinkedParcelCreateView.as_view(
                linked_model=Sponsoring,
                model=Parcel,
                form_class=UserParcelForm,
                template_name="parcel/user/update.html",
                success_url=lambda obj, kwargs: reverse(
                    "sponsor_parcel_tracking", kwargs={"lpk": obj.ownerId}
                ),
            )
        ),
